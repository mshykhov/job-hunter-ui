...
export const DefaultPromptsCard = ({
  form,
  defaultCL,
  defaultRM,
  onUpdate,
}: DefaultPromptsCardProps) => (
    ...
    <Flex vertical gap={12}>
      ...
      <Flex vertical gap={4}>
        ...
        <Input.TextArea
          ...
          onChange={(e) =>
            onUpdate((prev) => ({ ...prev, coverLetterPrompt: e.target.value || null }))
          }
          style={!form.coverLetterPrompt ? { opacity: 0.45 } : undefined}
          onFocus={(e) => {
            if (!form.coverLetterPrompt)
              onUpdate((prev) => ({ ...prev, coverLetterPrompt: defaultCL }));
            e.target.select();
          }}
        />
      </Flex>
      <Flex vertical gap={4}>
        ...
        <Input.TextArea
          ...
          onChange={(e) =>
            onUpdate((prev) => ({ ...prev, recruiterMessagePrompt: e.target.value || null }))
          }
          style={!form.recruiterMessagePrompt ? { opacity: 0.45 } : undefined}
          onFocus={(e) => {
            if (!form.recruiterMessagePrompt)
              onUpdate((prev) => ({ ...prev, recruiterMessagePrompt: defaultRM }));
            e.target.select();
          }}
        />
      </Flex>
