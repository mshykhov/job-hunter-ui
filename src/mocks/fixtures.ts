...
const TITLES = [
  "Java Full Stack Developer", "Senior Java IVR Application Developer", "Software Engineer (Java)",
  "Kotlin Backend Engineer", "Senior Backend Developer (Spring Boot)", "Lead Java Engineer",
  "Full Stack Engineer (React + Java)", "Platform Engineer", "Backend Developer (Microservices)",
  "Staff Software Engineer", "Java Team Lead", "Cloud Backend Engineer (AWS)",
  "Senior Software Engineer, Payments", "Distributed Systems Engineer", "Java Developer (Fintech)",
  "Principal Engineer", "Middle Java Developer", "Backend Engineer (Kafka, Spring)",
  "Senior Kotlin Developer", "API Platform Engineer",
];
const COMPANIES = [
  "Miratech", "Kozak Agency", "EPAM", "SoftServe", "Revolut", "Wise", "Monobank", "GlobalLogic",
  "Grammarly", "Bolt", "Playtika", "Ajax Systems", null, "N-iX", "Luxoft",
];
const SALARIES = [null, "$4000-6000", "€60k-80k", "$5500", "€70k-95k", null, "$3000-4500", null];
const LOCATIONS = [
  ["Kyiv"], ["Remote"], ["Warsaw", "Remote"], ["Lviv"], ["Berlin", "Remote"], ["Kyiv", "Remote"],
  ["London"], ["Amsterdam", "Remote"], ["Wroclaw"], ["Remote", "EU"],
];
...
export const buildDetail = (g: JobGroup): JobGroupDetail => ({
  groupId: g.groupId, title: g.title, company: g.company, remote: g.remote, status: g.status,
  ...
  jobs: Array.from({ length: g.jobCount }, (_, j) => ({
    jobId: `${g.groupId}-j${j}`, url: "https://example.com/job", source: g.sources[j % g.sources.length],
    description: DESCRIPTION, salary: g.salary, location: g.locations[0] ?? null, remote: g.remote,
    coverLetter: null, recruiterMessage: null, publishedAt: g.publishedAt, scrapedAt: g.matchedAt,
  })),
});

export const PUBLIC_JOBS: PublicJob[] = GROUPS.map((g) => ({
  id: g.id, title: g.title, company: g.company, url: "https://example.com/job",
  description: DESCRIPTION, source: g.sources[0], salary: g.salary, location: g.locations[0] ?? null,
  remote: g.remote, publishedAt: g.publishedAt, scrapedAt: g.matchedAt,
}));
